...
from .Common import xavier_normal_small_init_, xavier_uniform_small_init_


# Model definition
def make_model(d_atom, N=8, d_model=1024, h=16, dropout=0.0,
               lambda_attention=0.33, lambda_distance=0.33,
               trainable_lambda=False, N_dense=1, leaky_relu_slope=0.1,
               aggregation_type='mean', dense_output_nonlinearity='relu',
               distance_matrix_kernel='exp', use_edge_features=False,
               n_output=1, control_edges=False, integrated_distances=False,
               scale_norm=False, init_type='uniform', use_adapter=False):
    "Helper: Construct a model from hyperparameters."
    c = copy.deepcopy
    attn = MultiHeadedAttention(
        h, d_model, dropout, lambda_attention, lambda_distance,
        trainable_lambda, distance_matrix_kernel, use_edge_features,
        control_edges, integrated_distances)
    ff = PositionwiseFeedForward(
        d_model, N_dense, dropout, leaky_relu_slope, dense_output_nonlinearity)
    model = GraphTransformer(
        Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout, scale_norm,
                             use_adapter), N, scale_norm),
...
class GraphTransformer(nn.Module):
    ...
    def forward(self, src, src_mask, adj_matrix, distances_matrix, edges_att):
        ...
        return self.encode(src, src_mask, adj_matrix, distances_matrix,
                           edges_att)

    def encode(self, src, src_mask, adj_matrix, distances_matrix, edges_att):
        return self.encoder(
            self.src_embed(src), src_mask, adj_matrix,
            distances_matrix, edges_att)
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, N, scale_norm):
        ...
        self.norm = ScaleNorm(layer.size) \
            if scale_norm else LayerNorm(layer.size)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout, scale_norm,
        ...
        self.sublayer = clones(SublayerConnection(
                               size, dropout, scale_norm, use_adapter), 2)
        self.size = size

    def forward(self, x, mask, adj_matrix, distances_matrix, edges_att):
        "Follow Figure 1 (left) for connections."
        x = self.sublayer[0](x, lambda x: self.self_attn(
                             x, x, x, adj_matrix, distances_matrix,
                             edges_att, mask))
...
def attention(query, key, value, adj_matrix, distances_matrix, edges_att,
              ...
              distance_matrix_kernel=None, use_edge_features=False,
              control_edges=False, eps=1e-6, inf=1e12):
    "Compute 'Scaled Dot Product Attention'"
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(
            mask.unsqueeze(1).repeat(1, query.shape[1],
                                     query.shape[2], 1) == 0, -inf)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, lambda_attention=0.3,
                 lambda_distance=0.3, trainable_lambda=False,
                 distance_matrix_kernel='softmax', use_edge_features=False,
        ...
        if trainable_lambda:
            ...
            lambda_adjacency = 1. - lambda_attention - lambda_distance
            self.lambdas = (lambda_attention, lambda_distance,
                            lambda_adjacency)
        ...
        if use_edge_features:
            ...
            self.edges_feature_layer = EdgeFeaturesLayer(
                d_model, d_edge, h, dropout)

    def forward(self, query, key, value, adj_matrix, distances_matrix,
                edges_att, mask=None):
        "Implements Figure 2"
        if mask is not None:
        ...
        query, key, value = \
            [f(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for f, x in zip(self.linears, (query, key, value))]

        # Prepare distances matrix
        distances_matrix = distances_matrix.masked_fill(
        ...
        if self.use_edge_features:
            if self.integrated_distances:
                edges_att = torch.cat(
                    (edges_att, distances_matrix.unsqueeze(1)), dim=1)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, N_dense, dropout=0.1, leaky_relu_slope=0.0,
        ...
        if dense_output_nonlinearity == 'relu':
            self.dense_output_nonlinearity = \
                lambda x: F.leaky_relu(x, negative_slope=self.leaky_relu_slope)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.linears)-1):
            x = self.dropout[i](
                F.leaky_relu(self.linears[i](x),
                             negative_slope=self.leaky_relu_slope))

        return self.dropout[-1](self.dense_output_nonlinearity(
            self.linears[-1](x)))
