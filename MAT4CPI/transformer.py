...
class GraphTransformer(nn.Module):
    ...
    def forward(self, src, src_mask, adj_matrix, distances_matrix,
                edges_att, protein_embedding):
        "Take in and process masked src and target sequences."
        return self.predict(self.encode(src, src_mask, adj_matrix, distances_matrix, edges_att), src_mask, protein_embedding)
    ...
    def predict(self, out, out_mask, protein_embedding):
        return self.generator(out, out_mask, protein_embedding)
...
class Generator(nn.Module):
    ...
    def __init__(self, d_model, aggregation_type='mean', n_output=1, n_layers=1,
        ...
        if n_layers == 1:
            self.proj = nn.Linear(d_model+768, n_output)
        else:
            self.proj = []
            for i in range(n_layers-1):
                self.proj.append(nn.Linear(d_model+768, d_model))
    ...
    def forward(self, x, mask, protein_embedding):
        ...
        if self.aggregation_type == 'mean':
            ...
            out_avg_pooling = torch.cat(
                [out_avg_pooling, protein_embedding], -1)
